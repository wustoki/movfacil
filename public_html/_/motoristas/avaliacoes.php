...
Class avaliacoes {
    ...
    public function get_media_avaliacoes($motorista_id){
        $query = "SELECT AVG(nota) as media FROM avaliacoes WHERE motorista_id = :motorista_id AND pessoa = 'motorista'";
        $stmt = $this->conexao->prepare($query);
        $stmt->bindParam(':motorista_id', $motorista_id);
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
    ...
    public function get_media_avaliacoes_cliente($cliente_id){
        $query = "SELECT AVG(nota) as media FROM avaliacoes WHERE cliente_id = :cliente_id AND pessoa = 'cliente'";
        $stmt = $this->conexao->prepare($query);
        $stmt->bindParam(':cliente_id', $cliente_id);
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
